...
import {
  ...
  getIjpsPaymentMethods,
  getIjpsRecentTransactions,
} from "./journal-queries";

// ---------------------------------------------------------------------------
// Data layer for the IJPS-specific page (/journals/ijps).
// All values are read-only. Expenses / Google Ads are not tracked in the IJPS
// database, so those sections keep sample placeholders until a data source is
// connected.
// ---------------------------------------------------------------------------

export type IjpsPageData = {
...
export async function getIjpsPageData(): Promise<IjpsPageData> {
  ...
  const paymentMethods = await getIjpsPaymentMethods();
  const recentTransactions = await getIjpsRecentTransactions(8);
  ...
  // Use the last 7 data points from real monthly trends, or sample if empty.
  ...
  const revenueBreakdownIjps = [
    ...
    },
    {
      ...
      pct: revenue.total
        ? Math.round((revenue.plagiarism / revenue.total) * 100)
        : 0,
      color: "#f59e0b",
    },
    {
  ...
  return {
    ijpsKpis,
    manuscriptOverview: manuscriptOverview.length
      ? manuscriptOverview
      : sampleManuscriptOverview,
    manuscriptsByStatus,
