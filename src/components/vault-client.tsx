...
export function VaultClient({ initial }: { initial: Vault[] }) {
  ...
  const items = initial.filter((v) => cat === "All" || v.category === cat);
  const strong = initial.filter((v) => v.strength === "strong").length;
  const weak = initial.filter((v) => v.strength === "weak").length;
  ...
  return (
    <div className="animate-fade-up space-y-5">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard label="Stored Credentials" value={`${initial.length}`} icon={<KeyRound className="h-5 w-5" />} accent="var(--brand-soft)" />
